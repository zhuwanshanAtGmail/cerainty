...
\subsection{Present Certainty Equivalent Value with Risk-free Bond Investment}The MRN makes bond investment decision by changing number of shares of his bond holding from $\beta_{i+1}$ to $\beta_{i}$.Note that $\beta_i$ is a trading policy that depends on the realization of uncertain operations cash flow up to period $i$. Since MRN completes all consumption by the end of period $0$, $\beta_0=0$. $\beta_{i+1}$  is the MRN's initial wealth. We assume that a bond share's value at time $n$ is 1, it becomes $R^{n-i}$ in period $i$ because its value grows by a factor of $R$ in each period. MRN's bond investment decision generates cash $(\beta_{i+1}-\beta_{i})R^{n-i}$ because he sells $ \beta_{i+1}-\beta_{i}$ in period $i$.   Thus, in period $i$ the MRN has consumption cash $f_i + (\beta_{i+1} - \beta_i)R^{n-i}$. The MRN has an time additive exponential utility, her utility function of period $i$ is    \begin{eqnarray}  u_{ni}(x) =\frac{\rho_i}{R^{n-i}} u(x/\rho_i).\end{eqnarray}$\frac{\rho_i}{R^{n-i}}$ is the weight of period $i$'s utility, it can be viewed as a risk-adjusted discounting factor. This discounting factor is convenient for us to derive the values of PCEV. Other discounting factor will be discussed later. For a given uncertain cash flow, the MRN's maximum expected utility is\begin{eqnarray} \label{eqn:bondInv-rho}\mathcal{U}_n(f_n, \cdots, f_0|\beta_{n+1}) = \max_{\beta_{i} \forall 1 \leq i \leq n, \beta_0} \{\E_n[ \sum_{i=0}^n u_{ni}(f_i + (\beta_{i+1}-\beta_i)R^{n-i})]\}\end{eqnarray}Since the MRN achieves the same maximum expected consumption utility through the PCEV of an uncertain cash flow, we have\begin{definition}\label{def:PCEV}The present certainty equivalent value, $PCEV_n$, at time $n$ of an uncertain operational cash flow is defined as\begin{eqnarray} \label{eqn:PCEV}\mathcal{U}_n(PCEV_n(f_n, \cdots, f_0|\beta_{n+1}), 0, \cdots, 0) = \mathcal{U}_n(f_n, \cdots, f_0|\beta_{n+1})\end{eqnarray}\end{definition}Using above definition, we first study the maximum utility and optimal bond investment with $0$ cash flow in each period. {\lemma \label{lem:maxU-cash0} The maximum utility of zero operations cash for given initial wealth $\beta_{n+1}$ and the optimal shares are\begin{eqnarray}\mathcal{U}_n(0,0,\cdots,0|\beta_{n+1}) &=& \frac{\sum_{i=0}^n \rho_i R^i}{R^n} u(\frac{R^n}{\sum_{i=0}^n \rho_iR^i}\beta_{n+1}) \label{eqn:U-cash0},\\\beta_i^* & = &\beta_{n+1} \frac{\sum_{j=0}^{i-1}\rho_j R^j}{\sum_{j=0}^n \rho_j R^j} \label{eqn:beta-0}.\end{eqnarray}}Note that the first order condition implies that at the optimum, the utility, in each period without time weight, is the same. Thus the maximum utility equals the period 0's utility multiplied by the sum of all the time weights. Lemma \ref{lem:maxU-cash0} allow us to characterize the PCEV of a uncertain cash flow at time $n$ only:\begin{lemma} \label{lem:pcev-Xn-gen}The $PCEV_n$ of an uncertain cash in initial period $n$ is\begin{eqnarray} \label{eqn:pcev-Xn-gen}PCEV_n(f_n,0,\cdots,0|\beta_{n+1}) = \varrho_n u^{-1}\big{(} \E_n[u(\frac{f_n}{\varrho_n})]\big{)} ,\end{eqnarray}and independent of $\beta_{n+1}$, where $\varrho_n = \sum_{i=0}^n \rho_i/R^{n-i}$ .\end{lemma}The effective risk tolerance $\varrho_n$ is the sum of discount risk tolerance in all periods. It reflects the MRNÕs ability to use bond investment spreading risk across time. If the MRN has more periods for future assumption, he is effectively less risk averse to the uncertainty cash flow now than the uncertainty cash flow in the future. This effective risk tolerance captures the inter-temporal risk that a decision maker prefers early resolution of uncertainties.  To complete the PCEV evaluation of a general cash flow, we develop the following recursive equations.{\lemma For a general cash flow, its PCEV is \begin{eqnarray}PCEV_n(f_n, \cdots, f_0|\beta_{n+1}) = PCEV_n(f_n+\frac{PCEV_{n-1} (f_{n-1}, \cdots, f_0)}{R}, 0, \cdots, 0), \label{eqn:pcev-recur-gen}\end{eqnarray}and independent of $\beta_{n+1}$. }We have shown that, using a risk adjusted discount factor for utility time weight, the PCEV of uncertain operational cash flows can be computed recursively by discounting future PCEV. We have completed the recursive formula for the PCEV under bond investments.  In the next section, we present the PCEV formulas under currency derivative investment.\subsection{Present Certainty Equivalent Value with Financial Hedging}The financial hedging is actions that a MRN engages in exchange rate derivative investment to reduce the risks of exchange rate fluctuation. When the MRN has cash flows from operations in both home country and foreign country, financial hedging will reduce his risks and increase the PCEV of his operations cash flows. In this section, we study the optimal exchange rate derivative that allows the MRN to achieve maximum consumption utility and the corresponding PCEV.  An exchange rate derivative bought in period $i$ can be represented as a payoff function in period $i-1$ contingent on the realization of exchange rate, i.e.,   $\alpha_i(S_{i-1})$. Since the exchange rate market is complete, the exchange rate derivative price in period $i$ is $\E_i^S[\alpha_i(S_{i-1})]/R$, where $\E_i^S$ represents the expectation condition on $S_i$ under exchange rate risk neutral measure. Using exchange rate derivatives, the consumption cash equals $f_{i} + \alpha_{i+1}(S_i)-\frac{\E_i^S[\alpha_i(S_{i-1})]}{R}$, the operational cash plus the payoff of past derivatives minus the price of future derivatives. As a result, the maximum utility of given operational cash flows is\begin{eqnarray} \label{eqn:maxU-FH}\mathcal{U}_n(f_{n},\cdots,f_0|\alpha_{n+1}(s_n) )  =\max_{\alpha_{i}(S_{i-1})} \{\E_{n}^{\xi,S}[\sum_{i=0}^n u_{ni}(f_{i} + \alpha_{i+1}(S_i)-\frac{\E_i^S[\alpha_i(S_{i-1})]}{R})  |S_n=s_n]\}\end{eqnarray} where $\alpha_{n+1}(s_n)$ is the carry-over derivative  that expires in period $n$. $\alpha_0 =0$ imposes no consumption after period $0$. The corresponding PCEV is then defined in (\ref{eqn:PCEV}) except the bond investment is replaced by exchange rate derivative investment. We first study the maximum utility of 0 cash flow and the corresponding optimal exchange rate derivatives.

\begin{lemma}\label{lem:0-cash-FH}For any $n \geq 1$,  the maximum utility of 0 cash flow is\begin{eqnarray} \label{eqn:maxUti-0cash-FH}\mathcal{U}_n(0,\cdots,0|\alpha_{n+1}(s_n)) = \varrho_n u(\frac{\alpha_{n+1}(s_n)}{\varrho_n} ).\end{eqnarray}The corresponding optimal derivative satisfies\begin{eqnarray} \frac{\rho_{n}}{\varrho_{n-1}}\alpha_n( s_{n-1})+ \frac{\E_n^S[\alpha_n( S_{n-1})]}{R} = \alpha_{n+1}(s_n). \label{eqn:optim-beta}\end{eqnarray}\end{lemma}
...
Since the MRN's objective is to maximize the PCEV of operational cash flows across periods, we study the operational decisions in each period in this section. Since PCEV is monotone increasing in cash flow of each period and the operational decisions of transshipment and production in one period have no impact on other periods, these operations decisions can be made independently to maximize each periods operational cash flow. In each period, the operational cash flow is the revenue {from} sales to both markets minus the cost of production and transportation, thus the operational cash flow maximization problem is:

\begin{eqnarray}f_i(s_i,\bold k, \xiv_i) &=& \max_{(\bold{z,x}) \in A (\bold {k, \xiv_i})} \{(p-c)z^{d}+ (p-cs_i-t)x^{d} + s_i(p-c)z^{o}+(s_i p-c-t)x^{o} \}\label{eqn:profit} \\ A(\bold{k,\xiv_i}) &=& \{ (\bold {z,x}): \; z^{j} + x^{l\neq j} \leq k^j \; \forall j\in \{d,o\}\; \forall l\in \{d,o\}, \; \bold z+ \bold x \leq \xiv_i,\; \bold z \geq 0, \; \bold x \geq 0\} \label{eqn:prodCt}\end{eqnarray}

We know characterize the maximum operational cash flow and its corresponding optimal production and transshipment decisions. 
...
Having characterized the properties of optimal price and production and transshipment decisions, we now analyze capacity decisions. Since the one-period profit function $f$ is jointly concave in capacity, Theorem~\ref{thm:capacity} holds too in the price model. 

In summary for the Price Model, structural properties (concavity and submodularity), similar to those in Basic Model, are proved for all decisions.  In addition, the optimal price is always market clearing.  The optimal sales targets are independent of production and transshipment.  Sales targets and total profits decrease in price sensitivity.
