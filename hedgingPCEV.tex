...
\ABSTRACT{%
A Multinational Risk-averse Newsvendor (MRN) produces goods at home and sells both overseas and at home, over multiple periods.  The MRN's risks are driven by uncertain exchange rate, as well as uncertain demand.  Intuitively exchange rate risk should be managed by a finance department as financial risk, while uncertain demand risk should be managed by an operations department as a part of operational risk.  Traditionally this is the practice of many firms.  In this paper, we consider both of these risks jointly and investigate the effectiveness of two specific alternatives that allow MRN to reduce the total risk and to maximize the present certainty equivalent value of the cash flows.
...
Both hedges are special cases of the model: {\em financial hedging} means that we produce in DF only, while allowed to use any financial contracts; {\em operational hedging} means that we produce both in DF and OF, but use no financial hedging.  To take into consideration the risk-averse attitude, the dynamics of hedging activities, the limitation of available resources, and the cost of transshipment, we make the following four assumptions, which generalize some other more restrictive assumptions in the literature.

\begin{itemize}
    \item {\em Assumption 1}: The MRN is risk-averse and her objective is to maximize the present certainty equivalent value (PCEV).  The risk-averseness is captured by  concave utility function that is used to derive PCEV; in particular, the exponential utility function is assumed.
    ...
    \item {\em Assumption 4}: Transshipment is costly.
\end{itemize}

Having introduced the problem, the rest of the paper seeks to analyze it.  As a background, Section \ref{sect:liter} contains discussion of literature. In Section \ref{sect:pcev} we discuss the properties of PCEV. In Section \ref{sect:basicModel} we present structural properties of optimal solution for the Basic Model; while in Section \ref{sect:priceModel}, we do so for the Price Model.  In Section \ref{sect:numer}, we analyze the results of numerical analysis and discuss the relative strengths and weaknesses of the financial and operational hedging in detail.  Finally, in Section \ref{sect:conclusion}, we present our conclusions.
...
Even though the utility approach is mostly used to study the consumption decisions in economics and finance literature, it is also used in a few papers to evaluate a company's value and to study the operatons decisions. For example, Bouakiz and Sobel \cite{Bouakiz1992} use the expected utility of the total net present value; while this approach does not meet the stationary and separability condition, it meets the continuous and monotonic conditions.  Sobel \cite{Sobel2007} discusses benefits and drawbacks of discounted sum utilities vs. utility of net present value. However, using utility approach to evaluate a company's value by considering the operations decisions only has grave problems because it fails to account for the timing of uncertainty resolution, see Matheson and Howard (1968) and Mossion (1969). Furthermore, in the case of using expected utility of the total net present value, the optimal operations decisions are not time consistent.


Two modified utilty based approaches are developed to address the problems related to time consistency of decisions and to timing of uncertainty resolutions: recursive utilty function and present certainty equivalent value (PCEV). Kreps and Porteus (1978) first proposed recursive utility procedure to account for the tiing of uncertainty resolution. Epstein and Zin (1989) generalized their models to allow independent risk-aversion specifications for a given period and across periods. The Epstein-Zin framework has become the most popular model for dynamic asset pricing in economic and finance literature; it is discussed in textbooks including Singleton (2006) and Duffie (2001).  Present certianty equivalent value approach directly models the decision maker's decisions in both consumption and operations. Dreze and Modigliani (1972) studied the optimal consumption decision for a two-period uncertain income. Smith (1998) extended their model to multi-period and developed a recursive formula for PCEV using exponential utilty function. This recursive PCEV can be viewed as a special case of the recusrive utility in Epstein and Zin's framework. In our paper, we follow the most popular and most agreed-for discounted utility model and use PCEV to study the firm's decisions.
...
\section{Present Certainty Equivalent Value \label{sect:pcev}}

We use present certainty equivalent value (PCEV) to value the MRN's operations under various decisions because PCEV captures the decisions maker's risk preference that is sensitive to both uncertainty and the time at which the uncertainty is resolved. The standard expected utility method does not capture the impact of the timing of uncertainty resolution, as illustrated by an example in Smith (1998, page 1690), which is referred as the ``temporal risk problem''. While the recursive utility theorem by Epstein and Zin (1989) solves this problem, it is not in general consistent with the expected utility preference for consumption. The PCEV explicitly models the relevant borrowing and lending for consumption for a uncertain operational income cash flows, thus it is consistent with the consumption preference. This preference is modeled by an exponential utility function.


PCEV is the lump-sum amount received with certainty today that makes the decision maker just indifferent to receiving the uncertain operational cash flows. The decision maker is indifferent when the maximum utilities generate by the PCEV and by the uncertain operational cash flows are the same. The PCEV is to be evaluated in a period after the realization of exchange rate, but before demand realization. the other events and decisions in each period. (1) Uncertainty of exchange rate resolves. (2) Uncertainty of demand resolves. (3) Make production and transshipment decisions. (4) Collect operational cash flow. (5) Trade risk-free bond or trade exchange rate derivatives (Financial hedging) and obtain investment cash. (6) Consume cash.

We first derive the PCEV properties with risk-free bond trading and later with exchange rate derivative trading.


\subsection{PCEV with Risk-free Bond}

We let $n$ be the index of the current periods and count the periods backwards, the operations and consumption end in period 0. Let $f_i$ and $\beta_i$ be the cash generated in period $i$, respectively, from operations and from past investment. The cash consumption is period $i$ is then  $f_i + \beta_{i} -\beta_{i-1}/(1+r_f)$, where the last term is the price paid on the bond investment that pays cash $\beta_{i-1}$ in period $i-1$. Since consumption ends in period $0$, $\beta_{-1} =0$. Thus, for given initial wealth $\beta_{n}$,  the maximum consumption utility is
...
Since the decision maker is indifferent between PCEV and the operational cash flows, the maximum utility generated by PCEV is the same as the maximum utility generated by the operational cash flow, thus we have

\begin{definition}\label{def:PCEV}
The present certainty equivalent value at period $n$ of an uncertain operational cash flow $f_n, \cdots, f_0$ is defined as
\begin{eqnarray} \label{eqn:PCEV}
\mathcal{U}_n(PCEV_n(f_n, \cdots, f_0|\beta_{n}), 0, \cdots, 0|\beta_{n}) = \mathcal{U}_n(f_n, \cdots, f_0|\beta_{n}).
\end{eqnarray}
\end{definition}
...
Let the effective risk tolerance $R_i =\sum_{j=0}^i (\rho_j(1+r_f)^{j-i})$ and the time weight present value equivalent $w^\prime_i = \sum_{j=0}^i(\rho_j \ln((1+r_f)^j \rho_j)/w_j)/(1+r_f)^{i-j})$, then for a given initial wealth $\beta_n$ and a given uncertain operational cash flow with $f_n$ in period $n$ and $0$ in all other periods, the optimal bond investment satisfies
...
\begin{eqnarray}
PCEV_n(f_n,0,\cdots,0)=-R_n \ln\big{(} \E[\exp(-\frac{f_n}{R_n})]\big{)}, \; \forall n \geq 0. \label{eqn:pcev-Xn-gen}
\end{eqnarray}
\end{lemma}
...
PCEV_n(f_n, \cdots, f_0) = PCEV_n(f_n+\frac{PCEV_{n-1} (f_{n-1}, \cdots, f_0)}{1+r_f}, 0, \cdots, 0);
\end{eqnarray}
the optimal bond investment satisfies
\begin{eqnarray}\label{eqn:beta-gen}
\frac{\beta_{n-1} + w_{n-1}^\prime+PCEV_{n-1}(f_{n-1},\cdots,f_0)}{R_{n-1}} = \frac{f_n+\beta_n+ w_n^\prime -(\beta_{n-1} + w_{n-1}^\prime)/(1+r_f)}{\rho_n};
\end{eqnarray}
and the maximum utility is
\begin{eqnarray} \label{eqn:Un-gen}
\mathcal{U}_n(f_n,  \cdots, f_0|\beta_{n}) = \E[-R_n (1+r_f)^n \exp (-  (f_n + \beta_n + \frac{PCEV_{n-1}(f_{n-1},\cdots, f_0)}{1+r_f}+ w_n^\prime)/R_n)].
\end{eqnarray}
 }
...
When some uncertainties in the operational cash flow are due to exchange rate, the MRN can improve the PCEV by financial hedging, which is to buy foreign currency derivatives. This derivative is a future payoff as a function of the future realized value of the foreign currency. In order to obtain this future payoff, the MRN needs to pay the market price  of the derivatives, which is valued under risk neutral measure.
...
The corresponding PCEV is then defined in (\ref{eqn:PCEV}) except the bond investment is replaced by exchange rate derivative investment. This definition implies that the $PCEV_n(\cdot)$ is a function of $s_n$. If an operational cash flow has cash in period $n$ only and 0 in all other periods, the derivative investment reduces to bond investment. Consequently, Lemma~\ref{lem:pcev-Xn-gen} remains true under financial hedging.
...
PCEV_n(f_n, \cdots, f_0) = PCEV_n(f_n+\frac{\E[PCEV_{n-1} (f_{n-1}, \cdots, f_0)|S_n=s_n]}{1+r_f}, 0, \cdots, 0);
\end{eqnarray}
the optimal derivative investment satisfies
\begin{eqnarray}\label{eqn:beta-FH}
\frac{\beta_{n-1}(s_{n-1}) + PCEV_{n-1}(f_{n-1},\cdots,f_0|s_{n-1}) + w_{n-1}^\prime}{R_{n-1}} = \frac{f_n+\beta_n(s_n)+\frac{\E[\beta_{n-1}(S_{n-1})|S_n=s_n]}{1+r_f}+ w_n^\prime}{\rho_n};
\end{eqnarray}
and the maximum utility is
\begin{eqnarray} \label{eqn:Un-FH}
\mathcal{U}_n(f_n,  \cdots, f_0|\beta_{n}(s_n)) = \E[-R_n (1+r_f)^n \exp (-  \frac{f_n + \beta_n + \frac{\E[PCEV_{n-1}(f_{n-1},\cdots, f_0)|S_n=s_n]}{1+r_f}+ w_n^\prime}{R_n})].
\end{eqnarray}
 }
...
\end{eqnarray}
where the one period maximum operational cash is
\begin{eqnarray}
...
Having characterized the properties and optimal solutions of the production and transshipment decisions, we now proceed to study the properties of the optimal capcity decision that maximizes MRN’s PCEV.

\begin{theorem} \label{the:baseCap}
There is a unique optimal capacity allocation solution to (\ref{eqn:capacityAlloc}).
\end{theorem}

\proof It suffices to show that this maximization problem is equivalent to a problem of maximizing a concave function of $\bold{k}$ in that they have the same optimal $\bold{k}$. Since the utility function is increasing in consumption cash, (\ref{eqn:bondInv-rho}) and (\ref{eqn:Un-Xn}) implies that $\mathcal {U}_n(PCEV_n(f_n, \cdots, f_0), 0, \cdot, 0)$ is increasing in $PCEV_n(f_n, \cdots, f_0)$. As a result, maximizing $PCEV_n(f_n,\cdots, f_0)$ is equivalent to maximizing $\mathcal {U}_n(PCEV_n(f_n, \cdots, f_0), 0, \cdot, 0)$, and because of (\ref{eqn:PCEV}) it is equivalent to maximizing $\mathcal {U}_n(f_n, \cdots, f_0)$.
...
We prove the $PCEV_n(\cdot)$'s independence on $\beta_n$ by induction, the other results will follow from the induction proof process. We first apply variable substitution $\beta_i^\prime = \beta_i + w_i^\prime$ for all $i\in \{0,\cdots,n\}$. When $n=1$, we have
 \begin{equation*}
 \begin{array}{lll}
...
&=& \max_{\beta_0^\prime} \{\E [ -(1+r_f)\rho_1 \exp(- \frac{f_1 + \beta_1^\prime - \beta_0^\prime/(1+r_f)}{\rho_1}) - \rho_0 \exp(-\frac{PCEV_0(f_0)+ \beta_0^\prime}{\rho_0})]\},
 \end{array}
\end{equation*}
...
\[\frac{PCEV_0(f_0)+ \beta_0^\prime}{\rho_0}=\frac{f_1 + \beta_1^\prime - \beta_0^\prime/(1+r_f)}{\rho_1} .\]
Substituting back the original decisions, we get optimal investment decision (\ref{eqn:beta-gen}) for $n=1$, the corresponding maximum utility is then (\ref{eqn:Un-gen}). The PCEV in (\ref{eqn:pcev-gen}) follows from (\ref{eqn:PCEV}) and $PCEV_1(f_1,f_0|\beta_1$ is independent of $\beta_1$. This concludes the proof of induction  base, i.e., the truth for $n=1$.

Now suppose $PCEV_{n-1}(f_{n-1},\cdots, f_0|\beta_{n-1})$ is independent of $\beta_{n-1}$ for any $n-1 \geq 0$, we proceed to prove the $PCEV_n(\cdot)$'s independence on $\beta_n$ and all the other results.  By (\ref{eqn:bondInv-rho}) and variable subsitution, the left side of (\ref{eqn:Un-gen}) becomes
\[
...
\begin{equation*}
-(1+r_f)^n\rho_n \exp (-\frac{f_i + \beta_{i}^\prime -\beta^\prime_{n-1}/(1+r_f)}{\rho_n})-R_{n-1} (1+r_f)^{n-1} \exp (-  \frac{ \beta_{n-1}^\prime + PCEV_{n-1}(f_{n-1},\cdots, f_0)}{R_{n-1}}).
\end{equation*}
The first order optimality condition implies
\[\frac{\beta_{n-1}^\prime + PCEV_{n-1}(f_{n-1},\cdots, f_0)}{R_{n-1}}= \frac{f_n + \beta_{n}^\prime -\beta^\prime_{n-1}/(1+r_f)}{\rho_n} . \]
 ...
 \[\beta_{n-1}^\prime= (f_n + \beta_n^\prime - PCEV_{n-1}(f_{n-1},\cdots,f_0)\rho_n/R_{n-1})\frac{R_{n-1}}{R_n}\]

 This optimal decision implies that the equal adjusted cash consumption in period $n$ or $n-1$ is $ (f_n + \beta_n^\prime - PCEV_{n-1}(f_{n-1},\cdots,f_0)\rho_n/R_{n-1})/R_n+ PCEV_{n-1}(f_{n-1},\cdots, f_0)/R_{n-1}$. By simplification, it becomes
 \[
(f_n + \beta_n^\prime  + PCEV_{n-1}(f_{n-1},\cdots,f_0) \frac{(R_n-\rho_n)}{R_{n-1}})/R_n  = \frac{f_n + \beta_n^\prime - PCEV_{n-1}(f_{n-1},\cdots,f_0)/(1+r_f)}{R_n},
 \]
...
Finally,  using (\ref{eqn:PCEV}) we obtain (\ref{eqn:pcev-gen}) for $n$, which implies that $PCEV_n(\cdot)$ is independence of $\beta_n$.
\qed
...
\[-(1+r_f)\rho_1 \exp(- \frac{f_1 + \beta_1^\prime(s_1) - \frac{\E[\beta_0^\prime(S_0)|S_1=s_1]}{1+r_f}}{\rho_1}) - \E[\rho_0 \exp(-\frac{PCEV_0(f_0|S_0)+ \beta_0^\prime(S_0)}{\rho_0})|S_1=s_1]\}.
 \]
By point-wise maximization,  the first order optimality condition on $\beta_0^\prime(s_0)$ is
\[\frac{f_1 + \beta_1^\prime(s_1) - \E[\beta_0^\prime(S_0)|S_1=s_1]/(1+r_f)}{\rho_1} =\frac{PCEV_0(f_0|S_0=s_0)+ \beta_0^\prime(s_0)}{\rho_0}.\]
...
\[\frac{f_1 + \beta_1^\prime(s_1) - \E[\beta_0^\prime(S_0)|S_1=s_1]/(1+r_f)}{\rho_1} =\frac{\E[PCEV_0(f_0)|S_1=s_1]+ \E[\beta_0^\prime(S_0)|S_1=s_1]}{\rho_0}.\]
Simplifying this equations to obtain the price of the optimal derivative
\[ \frac{\E[\beta_0^\prime(S_0)|S_1=s_1]}{1+r_f} = (f_1 + \beta_1^\prime(s_1) -\E[PCEV(f_0)|S_1=s_1]\frac{\rho_1}{\rho_0}) \frac{\rho_0}{(1+r_f)R_1}.\]
...
\[\frac{f_1 + \beta_1^\prime(s_1)}{\rho_1} (1-\frac{\rho_0}{(1+r_f)R_1}) +\frac{\E[PCEV_0(f_0)|S_1=s_1]}{(1+r_f)R_1} = \frac{f_1 + \beta_1^\prime(s_1)+\E[PCEV_0(f_0)|S_1=s_1]/(1+r_f) }{R_1},\]
...
Now suppose $PCEV_{n-1}(f_{n-1},\cdots, f_0|\beta_{n-1}(s_{n-1}))$ is independent of $\beta_{n-1}(s_{n-1})$ for any $n-1 \geq 0$, we proceed to prove that $PCEV_n(f_n, \cdots, f_0|\beta_{n}(s_n))$ is independence of $\beta_n(s_n)$ and all the other results.  By (\ref{eqn:maxU-FH}) and variable subsitution, the left side of (\ref{eqn:Un-FH}) becomes
\[
...
&-R_{n-1} (1+r_f)^{n-1} \E[\exp (-  \frac{ \beta_{n-1}^\prime(S_{n-1}) + PCEV_{n-1}(f_{n-1},\cdots, f_0|S_{n-1})}{R_{n-1}})|s_n].
\end{align}
The first order optimality condition on $\beta^\prime(s_{n-1})$ is
\[ \frac{f_n + \beta_{n}^\prime(s_n) -\E[\beta^\prime_{n-1}(S_{n-1})|s_n]/(1+r_f)}{\rho_n} =\frac{\beta_{n-1}^\prime(s_{n-1}) + PCEV_{n-1}(f_{n-1},\cdots, f_0|s_{n-1})}{R_{n-1}}. \]
...
\[ \frac{f_n + \beta_{n}^\prime(s_n) -\E[\beta^\prime_{n-1}(S_{n-1})|s_n]/(1+r_f)}{\rho_n} =\frac{\E[\beta_{n-1}^\prime(S_{n-1})|s_n] + \E[PCEV_{n-1}(f_{n-1},\cdots, f_0)|s_n]}{R_{n-1}}. \]
This equation implies that the optimal derivative price is
\[
\frac{\E[\beta_{n-1}^\prime(S_{n-1})|s_n]}{1+r_f} = (f_n + \beta_{n}^\prime(s_n)-\E[PCEV_{n-1}(f_{n-1},\cdots, f_0)|s_n] \frac{\rho_n}{R_{n-1}})\frac{R_{n-1}}{(1+r_f)R_n}.
\]
This derivative price  implies that the consumption cash of period $n$ is
\[\frac{f_n + \beta_n^\prime(s_n)}{\rho_n} (1-\frac{R_{n-1}}{(1+r_f)R_n}) +\frac{\E[PCEV_{n-1}(f_{n-1}, \cdots, f_0)|s_n]}{(1+r_f)R_n} = \frac{f_n + \beta_n^\prime(s_n)+\frac{\E[PCEV_{n-1}(f_{n-1}, \cdots, f_0)|s_n]}{(1+r_f)} }{\rho_n},
\]
